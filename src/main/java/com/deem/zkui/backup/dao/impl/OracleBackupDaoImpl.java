 ...
 */
@Slf4j
public class OracleBackupDaoImpl extends AbstractBackupDao {

  public OracleBackupDaoImpl(Properties globalProps) {
    super(globalProps);
  }
  ...
  public List<BackData> queryAllBackData() {
    String sql =
        "select back_date,back_data from (select back_date, back_data from TMS_ZKUI_BACKUP order by back_date desc) where rownum<=10";
  ...
  protected String getCreateBackTableSQL() {
    ...
    sb.append("back_date number(15) primary key,");
    sb.append("back_data blob not null");
    sb.append(") tablespace tbs_alarm");
    return sb.toString();
  }
